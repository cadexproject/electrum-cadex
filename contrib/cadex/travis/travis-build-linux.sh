...
mkdir -p electrum-cadex/dist
wget -O electrum-cadex/dist/tor-proxy-setup.exe \
...
docker run --rm \
    ...
    -e PYHOME=$PYHOME \
    -v $(pwd):/opt \
    -v $(pwd)/electrum-cadex/:$WINEPREFIX/drive_c/electrum-cadex \
    -w /opt/electrum-cadex \
    -t zebralucky/electrum-cadex-winebuild:WinePy36 /opt/build_wine.sh
