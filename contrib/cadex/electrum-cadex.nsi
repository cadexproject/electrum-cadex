...
;Variables
  ...
  !define PRODUCT_NAME "Cadex Electrum"
  !define PRODUCT_NAME_NO_SPACE "Cadex-Electrum"
  !define PREV_PROD_NAME "Electrum-CADEX"
  !define PREV_PROD_NAME2 "Cadex-Electrum"
  !define PRODUCT_WEB_SITE "https://github.com/cadexdev/electrum-cadex"
...
;Interface Settings
  ...
  !define MUI_ICON "icons\electrum-cadex.ico"
...
Section "${PRODUCT_NAME}" SectionDE
  ...
  ${If} ${PREVINSTDIR} != ""
    ...
    DeleteRegKey HKCU "Software\Classes\cadex"
  ...
  File /r "dist\electrum-cadex\*.*"
  File "icons\electrum-cadex.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-cadex-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-cadex-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-cadex-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-cadex-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-cadex-${PRODUCT_VERSION}.exe" 0


  ;Links cadex: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\cadex" "" "URL:cadex Protocol"
  WriteRegStr HKCU "Software\Classes\cadex" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\cadex" "DefaultIcon" "$\"$INSTDIR\electrum-cadex.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\cadex\shell\open\command" "" "$\"$INSTDIR\electrum-cadex-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrum-cadex.ico"
...
;--------------------------------
;Descriptions
LangString DESC_DE ${LANG_ENGLISH} "Cadex Electrum Wallet"
