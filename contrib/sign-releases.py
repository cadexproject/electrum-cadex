...
To create a keystore run the following command:
    ...
    mkdir ~/.jks && keytool -genkey -v -keystore ~/.jks/keystore \
        -alias electrum.cadex.xyz -keyalg RSA -keysize 2048 \
        -validity 10000
...
Manual signing:
    ...
    jarsigner -verbose \
        ...
        -sigfile cadex-electrum \
        -keystore ~/.jks/keystore \
        Electrum_CADEX-3.0.6.1-release-unsigned.apk \
        electrum.cadex.xyz
...
Manual zip aligning:
    ...
    android-sdk-linux/build-tools/27.0.3/zipalign -v 4 \
        Electrum_CADEX-3.0.6.1-release-unsigned.apk \
        Cadex-Electrum-3.0.6.1-release.apk
...
SDIST_NAME_PATTERN = re.compile('^Cadex-Electrum-(.*).tar.gz$')
SDIST_DIR_TEMPLATE = 'Cadex-Electrum-{version}'
PPA_SOURCE_NAME = 'electrum-cadex'
...
UNSIGNED_APK_PATTERN = re.compile('^Electrum_CADEX-(.*)-release-unsigned.apk$')
SIGNED_APK_TEMPLATE = 'Cadex-Electrum-{version}-release.apk'
