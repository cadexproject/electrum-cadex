...
from electrum_cadex import cadex_tx
from electrum_cadex.bitcoin import (COIN, deserialize_privkey,
                                   b58_address_to_hash160,
                                   hash160_to_p2pkh, is_b58_address)
from electrum_cadex.crypto import hash_160
from electrum_cadex.cadex_tx import TxOutPoint
from electrum_cadex import ecc
from electrum_cadex.protx import ProTxMN, ProTxService, ProRegTxExc
from electrum_cadex.util import bfh, bh2u
...
class OutputsList(QListWidget):
    '''Widget that displays available 1000 KDX outputs.'''
...
class ImportLegacyWizardPage(QWizardPage):
    ...
    def update_lmn_data(self, current):
        ...
        if prevout_hash:
            val_cadex = '%s CADEX' % (value/COIN) if value else ''
            self.collateral_val.setText(val_cadex)
...
class BlsKeysWizardPage(QWizardPage):
    ...
    def initializePage(self):
        ...
        if start_id in parent.UPD_ENTER_PAGES:
            self.setSubTitle('Regenerate BLS keypair, setup cadexd')
            if not self.bls_priv.text():
                self.bls_priv.setText(new_mn.bls_privk)
                self.bls_pub.setText(new_mn.pubkey_operator)
        else:
            self.setSubTitle('Generate BLS keypair, setup cadexd')
    ...
    def generate_bls_keypair(self):
        ...
        self.bls_info_label.setText('BLS keypair generated. Before '
                                    ...
                                    'line to ~/.CADEXCOIN/cadexd.conf and '
                                    'restart masternode:')
...
class CollateralWizardPage(QWizardPage):
    ...
    def __init__(self, parent=None):
        ...
        self.not_found = QLabel('No 100000 KDX outputs were found.')
