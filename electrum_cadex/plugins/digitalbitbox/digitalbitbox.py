...
try:
    from electrum_cadex.crypto import Hash, EncodeAES, DecodeAES
    from electrum_cadex.bitcoin import (TYPE_ADDRESS, push_script, var_int, public_key_to_p2pkh, is_address,
                                  serialize_xpub, deserialize_xpub)
    from electrum_cadex import ecc
    from electrum_cadex.ecc import msg_magic
    from electrum_cadex.wallet import Standard_Wallet
    from electrum_cadex import constants
    from electrum_cadex.transaction import Transaction
    from electrum_cadex.i18n import _
    from electrum_cadex.keystore import Hardware_KeyStore
    from ..hw_wallet import HW_PluginBase
    from electrum_cadex.util import print_error, to_string, UserCancelled
    from electrum_cadex.base_wizard import ScriptTypeNotSupported, HWD_SETUP_NEW_WALLET
...
class DigitalBitbox_Client():
    ...
    def dbb_generate_wallet(self):
        ...
        filename = ("Cadex-Electrum-" + time.strftime("%Y-%m-%d-%H-%M-%S") + ".pdf")
        msg = ('{"seed":{"source": "create", "key": "%s", "filename": "%s", "entropy": "%s"}}' % (key, filename, 'Digital Bitbox Cadex Electrum Plugin')).encode('utf8')
