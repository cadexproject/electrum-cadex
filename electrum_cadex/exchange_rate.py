...
class ExchangeBase(PrintError):
    ...
    def get_json(self, site, get_string):
        ...
        response = requests.request('GET', url, headers={'User-Agent' : 'Cadex-Electrum'}, timeout=10)
        return response.json()

    def get_csv(self, site, get_string):
        url = ''.join(['https://', site, get_string])
        response = requests.request('GET', url, headers={
            'User-Agent': 'Cadex-Electrum'
        })
...
class BitcoinAverage(ExchangeBase):
    ...
    def get_rates(self, ccy):
        json = self.get_json('apiv2.bitcoinaverage.com',
                             '/indices/local/ticker/KDX%s' % ccy)
...
class Bittrex(ExchangeBase):
    def get_rates(self, ccy):
        json = self.get_json('bittrex.com',
                             '/api/v1.1/public/getticker?market=%s-KDX' % ccy)
        quote_currencies = {}
...
class Bittrex(ExchangeBase):
    def get_rates(self, ccy):
        json = self.get_json('https://cratex.io',
                             'api/v1/get_markets.php?market=KDX/BTC' % ccy)
        quote_currencies = {}
        if not json.get('success', False):
            return quote_currencies
        last = Decimal(json['result']['Last'])
...
class Poloniex(ExchangeBase):
    def get_rates(self, ccy):
        ...
        quote_currencies = {}
        cadex_ticker = json.get('BTC_KDX')
        quote_currencies['BTC'] = Decimal(cadex_ticker['last'])
        return quote_currencies


class CoinMarketCap(ExchangeBase):
    def get_rates(self, ccy):
        json = self.get_json('api.coinmarketcap.com', '/v1/ticker/kdx/')
        quote_currencies = {}
