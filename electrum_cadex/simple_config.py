...
class SimpleConfig(PrintError):
    ...
    def electrum_path(self):
        ...
        self.print_error("electrum-cadex directory", path)
        return path
    ...
    def convert_version_3(self):
        ...
        if isinstance(base_unit, str):
            ...
            map_ = {'kdx':8, 'mkdx':5, 'ukdx':2, 'duffs':0, 'sat':0}
...
def read_user_config(path):
    """Parse and store the user config settings in electrum-cadex.conf into user_config[]."""
    if not path:
        return {}
